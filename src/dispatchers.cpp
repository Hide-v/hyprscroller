...
int workspace_for_action() {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get()) return -1;
  ...
  if (workspace_id == WORKSPACE_INVALID) return -1;
...
SDispatchResult dispatch_cyclesize(std::string arg) {
  ...
  if (step != 0) g_ScrollerLayout->cycle_window_size(workspace, step);
...
SDispatchResult dispatch_cyclewidth(std::string arg) {
  ...
  int step = parse_step_arg(arg);
  if (step != 0) g_ScrollerLayout->cycle_window_width(workspace, step);
...
SDispatchResult dispatch_cycleheight(std::string arg) {
  ...
  int step = parse_step_arg(arg);
  if (step != 0) g_ScrollerLayout->cycle_window_height(workspace, step);
...
SDispatchResult dispatch_marksdelete(std::string arg) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error = "scroller:marksdelete: called while not running hyprscroller"};
...
SDispatchResult dispatch_marksvisit(std::string arg) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error = "scroller:marksvisit: called while not running hyprscroller"};
...
SDispatchResult dispatch_marksreset(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error = "scroller:marksreset: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailnew(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error = "scroller:trailnew: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailnext(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error = "scroller:trailnext: called while not running hyprscroller"};
...
SDispatchResult dispatch_traildelete(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error = "scroller:traildelete: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailclear(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error = "scroller:trailclear: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailmarkprev(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error =
                "scroller:trailmarkprevious: called while not running "
                "hyprscroller"};
