...
class Window {
 public:
  ...
  ~Window() { window->removeWindowDeco(decoration); }
  ...
  void push_fullscreen_geom() { push_geom(mem_fs); }
  void pop_fullscreen_geom() { pop_geom(mem_fs); }
  void push_overview_geom() { push_geom(mem_ov); }
  void pop_overview_geom() { pop_geom(mem_ov); }
  ...
  // Called by the parent column on the active window every time it changes
  // width This allows windows to have a independently stored width when they
  // leave the column
  ...
  bool is_window(PHLWINDOW w) const { return window == w; }
  ...
  void scale(const Vector2D &bmin, const Vector2D &start, double scale,
             double gap0, double gap1) {
    set_geom_h(get_geom_h() * scale);
    SBoxExtents reserved_area = window->getFullWindowReservedArea();
    window->m_vPosition =
        start + reserved_area.topLeft +
        (window->m_vPosition - reserved_area.topLeft - bmin) * scale;
    window->m_vPosition.y += gap0;
    window->m_vSize.x *= scale;
    window->m_vSize.y = (window->m_vSize.y + reserved_area.topLeft.y +
                         reserved_area.bottomRight.y + gap0 + gap1) *
                            scale -
                        gap0 - gap1 - reserved_area.topLeft.y -
                        reserved_area.bottomRight.y;
    window->m_vSize = Vector2D(std::max(window->m_vSize.x, 1.0),
                               std::max(window->m_vSize.y, 1.0));
    *window->m_vRealSize = window->m_vSize;
    *window->m_vRealPosition = window->m_vPosition;
    window->sendWindowSize();
  }

  void move_to_bottom(double x, const Box &max, const Vector2D &gap_x,
                      double gap) {
    SBoxExtents reserved_area = window->getFullWindowReservedArea();
    Vector2D topL = reserved_area.topLeft, botR = reserved_area.bottomRight;
    window->m_vPosition = Vector2D(x + topL.x + gap_x.x,
                                   max.y + max.h - get_geom_h() + topL.y + gap);
  }
  void move_to_top(double x, const Box &max, const Vector2D &gap_x,
                   double gap) {
    SBoxExtents reserved_area = window->getFullWindowReservedArea();
    Vector2D topL = reserved_area.topLeft, botR = reserved_area.bottomRight;
    window->m_vPosition = Vector2D(x + topL.x + gap_x.x, max.y + topL.y + gap);
  }
  void move_to_center(double x, const Box &max, const Vector2D &gap_x,
                      double gap0, double gap1) {
    SBoxExtents reserved_area = window->getFullWindowReservedArea();
    Vector2D topL = reserved_area.topLeft, botR = reserved_area.bottomRight;
    window->m_vPosition = Vector2D(
        x + topL.x + gap_x.x,
        max.y + 0.5 * (max.h -
                       (botR.y - topL.y + gap1 - gap0 + window->m_vSize.y)));
  }
  ...
  void update_window(double w, const Vector2D &gap_x, double gap0, double gap1,
    ...
    window->m_vSize =
        Vector2D(std::max(w - reserved.topLeft.x - reserved.bottomRight.x -
                              gap_x.x - gap_x.y,
                          1.0),
                 std::max(get_geom_h() - reserved.topLeft.y -
                              reserved.bottomRight.y - gap0 - gap1,
                          1.0));
    if (!animate) window->m_vRealPosition->warp(false);
    *window->m_vRealPosition = window->m_vPosition;
    *window->m_vRealSize = window->m_vSize;
    window->sendWindowSize();
  }
  bool can_resize_width(double geomw, double maxw, const Vector2D &gap_x,
                        double gap, double deltax) {
    ...
    auto mwidth = geomw + deltax - topL.x - botR.x -
                  2.0 * std::max(std::max(gap_x.x, gap_x.y), gap);
    if (mwidth <= 0.0 || rwidth >= maxw) return false;

    return true;
  }
  bool can_resize_height(double maxh, bool active, double gap0, double gap1,
                         double deltay) {
    SBoxExtents reserved_area = window->getFullWindowReservedArea();
    const Vector2D topL = reserved_area.topLeft,
                   botR = reserved_area.bottomRight;
    auto wh = get_geom_h() - gap0 - gap1 - topL.y - botR.y;
    if (active) wh += deltay;
    if (wh <= 0.0 || wh + gap0 + gap1 + topL.y + botR.y > maxh) return false;
    return true;
  }

  CGradientValueData get_border_color() const;

  void selection_toggle() { selected = !selected; }

  void selection_set() { selected = true; }

  void selection_reset() {
    if (selected) selection_toggle();
  }
 ...
 private:
  ...
  double geom_w;
  double box_h;
  Memory mem_ov, mem_fs;  // memory to store old height and win y when in
                          // overview/fullscreen modes
  bool selected;
